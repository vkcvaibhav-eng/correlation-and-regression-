...
st.set_page_config(page_titleimport io
...
="Statistical Analysis App", layout="wide")
...
def build_equation_from_model(model, target_var):
    ...
    if not eq_parts:
        return f"{target_var} = NA"
    return f"{target_var} = " + " ".join(eq_parts)
...
def build_year_wise_regression_summary(df, dependent_var, weather_vars):
    ...
    for year, year_df in working_df.groupby("Year", sort=True):
        ...
        try:
            ...
            rows.append({
                "Year": year_label,
                "Regression equation": build_equation_from_model(model, dependent_var),
                "R-squared": model.rsquared,
                "Adjusted R-squared": model.rsquared_adj,
                "F-statistic": model.fvalue,
                "Prob(F)": model.f_pvalue,
    ...
    return format_regression_summary_df(pd.DataFrame(rows)), warnings
...
def render_multi_year_analysis(df):
    ...
    st.subheader("SECTION 6: Year-wise regression equations")
    year_regression_df, regression_warnings = build_year_wise_regression_summary(
        analysis_df,
        dependent_var,
        weather_vars,
    )
